...
const LOG_INTERVAL = 100; // 0.1 sec
const FILE_LOG_INTERVAL = 1000 * 60; // 1 min
...
const logSingleLine = (text) => {
  ...
  const output = text.length <= columns
    ? text.replace(/(?:\r\n|\r|\n)/g, "")
...
let totalTime = 0;
setInterval(() => {
  totalTime += LOG_INTERVAL;
  childProcess.exec(command, (error, stdout) => {
    if (error !== null) activityMonitor.emit(EVENTS.ERROR, error);

    activityMonitor.emit(EVENTS.LOG, stdout);

    // dummy assumption that they're multiples for simplicity
    const shouldLogToFile = totalTime % FILE_LOG_INTERVAL === 0;
    if (shouldLogToFile) activityMonitor.emit(EVENTS.FILE, stdout);
  });
}, LOG_INTERVAL);
